...
const { TIMERS, HIDE_FROM, msg, getUserById, nullify } = require('../utils');

module.exports = () => {
  // Game Stages //

  // Stages are not analagous to game rounds.
  // Stages are listed in the order they (generally) occur in game.
  ...
  function advanceStage(game, stageId, io) {
    if (stageId) {
      // Advance directly to given stage.
      game.currentStage = STAGES.find((s) => s.id === stageId);
    } else {
      // Advance to the next stage in the STAGES array.
      const stageNum = game.currentStage.stageNum;
      console.log(stageNum);
      game.currentStage = STAGES[stageNum];
    }

    if (game.settings.timer.on) handleTimer(game, io);
  }
  ...
  function advanceOnKeyEvChosen(game, io, keyEv) {
    console.log('advKeyEv', game.currentStage, keyEv);
  ...
  function confirmClueChoice(game, clue) {
    function lockClueCard() {
      ...
      const card = game.cluesDeck.find((c) =>
        c.opts.some((o) => o.id === clue),
      );
      card.isLocked = true;
    }
  ...
  function resolveAccusal(game, accusalEv, accuser, io) {
    ...
    function advToSecondMurder() {
      advanceStage(game, 'second-murder', io);
    ...
    function resolveWrongAccusal() {
      return game.blueTeam.some((player) => !!player.canAccuse)
        ? continueRound()
    ...
    function continueRound() {
      const args = [[accuser.id, accuser.color.id]];
  ...
  function resolveGame(game, result, args, io) {
    advanceStage(game, 'game-over', io);
  ...
  function redact(game, role) {
    game.viewingAs = role;
    return nullify(game, HIDE_FROM[role]);
  }
